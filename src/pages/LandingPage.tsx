...
import { FiMenu, FiX, FiHome, FiBookOpen, FiPhone, FiInfo, FiFileText, FiShield, FiLogOut, FiUser, FiSettings, FiUserCheck, FiUsers, FiMapPin, FiHelpCircle, FiTruck } from 'react-icons/fi';
import '@fortawesome/fontawesome-free/css/all.min.css';

const LandingPage = () => {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [userRole, setUserRole] = useState<string | null>(null); // Set user role here (e.g., 'admin', 'user')

  React.useEffect(() => {
    const role = localStorage.getItem('userRole'); // Assuming userRole is stored in localStorage after login
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      {/* Sidebar */}
      <aside className={`bg-gray-800 text-white w-64 p-4 flex flex-col fixed h-full transition-transform duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 z-20 bg-cover bg-center`}>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold">Nazarene Vehicle Rental</h2>
        ...
        <nav className="flex flex-col gap-2">
          ...
          <Link to="/booking" className="flex items-center hover:bg-gray-700 p-2 rounded">
            <FiBookOpen className="mr-2" />
            Book a Vehicle
          </Link>
          <Link to="/contact" className="flex items-center hover:bg-gray-700 p-2 rounded">
            <FiPhone className="mr-2" />
            Contact Us
          </Link>
          <Link to="/about" className="flex items-center hover:bg-gray-700 p-2 rounded">
            <FiInfo className="mr-2" />
            About Us
          </Link>
          <Link to="/terms" className="flex items-center hover:bg-gray-700 p-2 rounded">
            <FiFileText className="mr-2" />
            Terms of Service
          </Link>
          <Link to="/privacy" className="flex items-center hover:bg-gray-700 p-2 rounded">
          ...
          {userRole && (
            <>
              ...
              <Link to="/user-dashboard" className="flex items-center hover:bg-gray-700 p-2 rounded">
                <FiUser className="mr-2" />
                Dashboard
              </Link>
              <Link to="/settings" className="flex items-center hover:bg-gray-700 p-2 rounded">
                <FiSettings className="mr-2" />
                Account Settings
              </Link>
              <Link to="/history" className="flex items-center hover:bg-gray-700 p-2 rounded">
                <FiBookOpen className="mr-2" />
                Booking History
              </Link>
            </>
          ...
          {userRole === 'admin' && (
            <>
              ...
              <Link to="/manage-vehicles" className="flex items-center hover:bg-gray-700 p-2 rounded">
                <FiTruck className="mr-2" />
                Manage Vehicles
              </Link>
              <Link to="/manage-users" className="flex items-center hover:bg-gray-700 p-2 rounded">
                <FiUserCheck className="mr-2" />
                Manage Users
              </Link>
              <Link to="/reports" className="flex items-center hover:bg-gray-700 p-2 rounded">
                <FiFileText className="mr-2" />
                Reports
              </Link>
              <Link to="/locations" className="flex items-center hover:bg-gray-700 p-2 rounded">
                <FiMapPin className="mr-2" />
                Locations
              </Link>
              <Link to="/support-tickets" className="flex items-center hover:bg-gray-700 p-2 rounded">
        ...
        {userRole ? (
          <button
            ...
            className="mt-auto bg-red-500 hover:bg-red-600 p-2 rounded flex items-center justify-center"
          >
            <FiLogOut className="mr-2" />
            Logout
          </button>
        ) : (
          <Link to="/register" className="mt-auto bg-green-500 hover:bg-green-600 p-2 rounded flex items-center justify-center">
            <FiUser className="mr-2" />
            Register
          </Link>
        )}
      ...
      {/* Header */}
      <header className="bg-white shadow-md p-4 flex justify-between items-center w-full fixed top-0 z-10 md:ml-64">
